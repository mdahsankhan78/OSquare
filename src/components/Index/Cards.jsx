    ...
    return (
        <>
            <div className="grid grid-cols-3 gap-4">
                <Link to={`/profile/${name}/${email}`} className="flex flex-col items-center rounded-2xl shadow-card p-4">
                    <img src="/images/icon-images/profile.png" className='h-12' alt="" />
                    <h1 className='text-lg text-foreground'>Profile</h1>
                </Link  >

                <Link to={`/dashboard/${name}/${email}`} className="flex flex-col items-center rounded-2xl shadow-card p-4">
